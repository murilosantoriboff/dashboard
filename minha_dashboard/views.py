from .models import Vendas
from .models import Produto
from .models import Vendedor
from django.shortcuts import render
from django.http import HttpResponse
from django.http import JsonResponse
from django.db.models import Sum
from datetime import datetime

def home(request):
    return render(request, 'home.html')

def retorna_total_vendido(request):
    total = Vendas.objects.all().aggregate(Sum('total'))['total__sum']
    if request.method == 'GET':
        return JsonResponse({'total':total})

def relatorio_faturamento(request):
    r = Vendas.objects.all()

    meses = ['jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez']
    data = []
    labels = []
    cont = 0
    mes = datetime.now().month + 1
    ano = datetime.now().year
    for i in range(12):
        mes -= 1
        if mes == 0:
            mes = 12
            ano -= 1
        soma = sum([i.total for i in r if i.data.month == mes and i.data.year == ano])
        labels.append(meses[mes-1])
        data.append(soma)
        cont += 1
    data_json = {'data':data[::-1], 'labels':labels[::-1]}
    return JsonResponse(data_json)

def relatorio_produtos(request):
    prod = Produto.objects.all()
    label = []
    data = []
    for pro in prod:
        vendas = Vendas.objects.filter(nome_produto=pro).aggregate(Sum('total'))
        if not vendas['total__sum']:
            vendas['total__sum']
        label.append(pro.nome)
        data.append(vendas['total__sum'])

    x = list(zip(label, data))

    x.sort(key=lambda x: x[1], reverse=True)
    x = list(zip(*x))

    return JsonResponse({'labels':x[0][:3], 'data':x[1][:3]})

def relatorio_funcionario(request):
    funci = Vendedor.objects.all()
    label = []
    data = []
    for fun in funci:
        vendas = Vendas.objects.filter(vendedor=fun).aggregate(Sum('total'))
        if not vendas['total__sum']:
            vendas['total__sum'] = 0
        label.append(fun.nome)
        data.append(vendas['total__sum'])
        x = list(zip(label,data))

        x.sort(key=lambda x: x[1], reverse=True)
        x = list(zip(*x))
        return JsonResponse({'labels': x[0][:3], 'data':x[1][:3]})